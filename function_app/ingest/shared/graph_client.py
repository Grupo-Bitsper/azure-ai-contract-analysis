...
def create_subscription(
    ...
    """Crea una Graph subscription para /drives/{drive_id}/root."""
    resp = requests.post(
    ...
    if not resp.ok:
        raise RuntimeError(
            f"Graph POST /subscriptions failed {resp.status_code}: {resp.text[:500]}"
        )
    return resp.json()


def renew_subscription(subscription_id: str, expiration_dt: str) -> dict:
    resp = requests.patch(
        f"{GRAPH_BASE}/subscriptions/{subscription_id}",
        headers={**_headers(), "Content-Type": "application/json"},
        json={"expirationDateTime": expiration_dt},
        timeout=REQUEST_TIMEOUT,
    )
    if not resp.ok:
        raise RuntimeError(
            f"Graph PATCH /subscriptions/{subscription_id} failed "
...
def delete_subscription(subscription_id: str) -> None:
    ...
    if not resp.ok and resp.status_code != 404:
        raise RuntimeError(
            f"Graph DELETE /subscriptions/{subscription_id} failed "
            f"{resp.status_code}: {resp.text[:200]}"
        )
